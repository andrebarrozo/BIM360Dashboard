...
namespace bim360issues.Controllers
{
    public class BIM360Controller : ControllerBase
    {
        ...
        public async Task<IRestResponse> GetIssuesAsync(string containerId, string resource, string filterType, string filterValue)
        {
            Credentials credentials = await Credentials.FromSessionAsync(base.Request.Cookies, Response.Cookies);
            

            RestClient client = new RestClient(BASE_URL);
            RestRequest request = new RestRequest("/issues/v1/containers/{container_id}/{resource}?filter[{filterType}]={filterValue}", RestSharp.Method.GET);
        ...
        [HttpGet]
        [Route("api/forge/bim360/account/{accountId}/container/{containerId}")]
        public async Task<JArray> GetIssuesAsync(string accountId, string containerId, string urn)
        {
            DateTime sixMonthsAgo = DateTime.Now.Subtract(new TimeSpan(180, 0, 0, 0, 0));
            IRestResponse documentIssuesResponse = await GetIssuesAsync(containerId, "quality-issues", "created_at", sixMonthsAgo.ToString("YYYY-MM-DDThh:mm:ss.sz"));
        ...
        {
            IRestResponse documentIssuesResponse = await GetIssuesAsync(containerId, "quality-issues", "target_urn", Encoding.UTF8.GetString(Convert.FromBase64String(urn)));
            IRestResponse usersResponse = await GetUsers(accountId);
